...
SELECT COALESCE(t.user_id, y.user_id) AS user_id,
    CASE
        ...
        WHEN y.dates_active IS NULL THEN ARRAY [t.date_active]                                              --  if yesterday's activity is null, use today's activity
        WHEN t.date_active IS NULL THEN y.dates_active                                                      --  if today's activity is null, use yesterday's activity
        ELSE ARRAY [t.date_active] || y.dates_active                                                        --  Otherwise, append today’s date to existing dates
    END AS dates_active,
    COALESCE(t.date_active, y.date + INTERVAL '1 day') AS date                                              -- advance the snapshot date
FROM today t
